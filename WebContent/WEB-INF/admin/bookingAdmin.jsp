...
<title>시간표 관리</title>
   ...
   <script type="text/javascript">
	   	function ch_title(){
		    document.myform.action="bookingFind.do";
...
<!-- ******************************************************************************** -->
	...
	<div>
		<%
			ArrayList<BookingEntity> booklist = (ArrayList<BookingEntity>)request.getAttribute("bookingList");
		%>
		<form action="bookingDelete.do" name=myform>
			<table class="table">
			<thead>
				<tr>
					<th>예약번호</th>
				...
				<tbody>
				<%
					for(BookingEntity book : booklist) {
				%>
				<tr>
					<td><%=book.getBookingno()%></td>
					<td><%=book.getBvo().getMemberno()%></td>
					<td><%=book.getBvo().getPhonenumber()%></td>
					<td><%=book.getBvo().getTimeno()%></td>
					<td><%=book.getBvo().getMovieCd()%></td>
					<td><%=book.getBvo().getCinemaID()%></td>
					<td><%=book.getBvo().getScreenID()%></td>
					<td><input type="checkbox" name="ch" value="<%=book.getBookingno()%>"/>						 
				  </td>
				</tr>
				<%
					}
				%>
				<tr>
					<td align="center" colspan="3"><a href="bookingInsert.do">추가</a></td>
		...
		<form:form method="post" enctype="multipart/form-data" modelAttribute="uploadFile" action="BookingUpload.do">
