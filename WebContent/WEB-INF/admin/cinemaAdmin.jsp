...
<title>관리자 메인</title>
    ...
    <script type="text/javascript">
	    function ch_title(){
		   document.myform.action="cinemaFind.do";
...
<div class="topspace" style="padding:15px; background:#111; height:100px"></div>
	<div>
		<%
			ArrayList<CinemaEntity> cinemalist = (ArrayList<CinemaEntity>)request.getAttribute("cinemaList");
		%>
		<form action="cinemaDelete.do" name=myform>
			<table class="table">
				<thead>
					<tr>
						<th>영화관ID</th>
				...
				<tbody>
					<%
						for(CinemaEntity res : cinemalist) {
					%>
					<tr>
						<td><%=res.getCinemaID()%></td>
						<td><%=res.getCinemaVO().getCinemaname()%></td>
						<td><%=res.getCinemaVO().getBrand()%></td>
						<td><%=res.getCinemaVO().getLoc()%></td>
						<td><input type="checkbox" name="ch" value="<%=res.getCinemaID()%>"/>						 
					  </td>
					</tr>
					<%
						}
					...
					<tr>
						...
						<!-- <td align="center" colspan="3"><a href="cinemaInsert.do">추가</a></td>-->						
			...
			<form:form method="post" enctype="multipart/form-data" modelAttribute="uploadFile" action="CinemaUpload.do">
