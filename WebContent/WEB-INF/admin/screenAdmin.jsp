<%@page import="com.entity.ScreenEntity"%>
...
<title>관리자 메인</title>
    ...
    <script type="text/javascript">
	    function ch_title(){
		   document.myform.action="screenFind.do";
...
<div class="topspace" style="padding:15px; background:#111; height:100px"></div>
	<div>
		<%
			ArrayList<ScreenEntity> screenlist = (ArrayList<ScreenEntity>)request.getAttribute("screenlist");
		%>
		<form action="screenDelete.do" name=myform>
			<table class="table">
				<thead>
					<tr>
						<th>영화관ID</th>
				...
				<tbody>
					<%
						for(ScreenEntity res : screenlist) {
					%>
					<tr>
						<td><%=res.getCinemaID()%></td>
						<td><%=res.getScreenID()%></td>
						<td><input type="checkbox" name="ch" value="<%=res.getScreenID()%>"/>						 
					  </td>
					</tr>
					<%
						}
			...
			<form:form method="post" enctype="multipart/form-data" modelAttribute="uploadFile" action="ScreenUpload.do">
