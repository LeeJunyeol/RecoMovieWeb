<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page	import="java.util.*,com.entity.Board, com.entity.MemberEntity"%>
...
.updateinsert{margin-top:20px;} 
.updateinsert {margin-right:20px; width:8%;margin:0 auto;padding:0.7em 1.5em 0.8em 1.5em;font-size:16px;color:#000000;border:1px solid #008d2d;border-radius:2em;background:#DADADA; text-align:center;letter-spacing:-0.5px; font-weight:bold;}
.updateinsert:hover{background:#fff;color:#676767;}
.updateinsert{margin-top:20px;}  

.reset {margin-left:5%;width:8%;margin:0 auto;padding:0.7em 1.5em 0.8em 1.5em;font-size:16px;color:#000000;border:1px solid #008d2d;border-radius:2em;background:#DADADA; text-align:center;letter-spacing:-0.5px; font-weight:bold;}
.reset:hover{background:#fff;color:#676767;}

.updatedelete {margin-left:5%;width:8%;margin:0 auto;padding:0.7em 1.5em 0.8em 1.5em;font-size:16px;color:#000000;border:1px solid #008d2d;border-radius:2em;background:#DADADA; text-align:center;letter-spacing:-0.5px; font-weight:bold;}
.updatedelete:hover{background:#fff;color:#676767;}
...
<!-- ******************************************************************************** -->
	<div align="center" style="padding-top: 200px;">
		<%	//배달한 값을 찾아서 출력
			Board board = (Board)request.getAttribute("find");
			MemberEntity member = (MemberEntity)session.getAttribute("me");
		%>
			...
			<form method="post" action="BoardUpdate.do">
				<table class="insertboard" width="100%">
					...
					<tr>
						<th class="th" scope="row"><img
							src="img/insertboard/inserttitle1.PNG" /></th>
					...
					<tr>
						<th class="th" scope="row"><img
							src="img/insertboard/insertname1.PNG" alt="이름" /></th>
						<td><a><%=board.getName()%></a></td>
						<input type="hidden" name=name class="wid20" size=20 maxlength=20
							value="<%=board.getName()%>">
					</tr>

					<tr>
						<th class="th" scope="row"><img
							src="img/insertboard/inserthand1.PNG" alt="핸드폰번호" /></th>
						...
						</select><input type="text" title="전화번호 앞" maxlength="4" lass="wid20" /> <input
							type="text" title="전화번호 뒤" maxlength="4" class="wid20" /></td>
					</tr>

					<tr>
						<th class="insertemail" scope="row"><img
							src="img/insertboard/insertemail1.PNG" alt="이메일" /></th>
					...
					<td><input type="password" name="password" id="writepassword"maxlength="10"></td>
				</tr>

					<tr>
						<th class="th" scope="row"><img
							src="img/insertboard/insertcontent1.PNG" alt="내용" /></th>
						<td><textarea name=contents cols=50 rows=10 title="Q&A"><%=board.getContents()%></textarea></td>
					</tr>
				</table>
				...
				<div class=udbtgr>
					...
					<%if(!board.getName().equals(member.getId())){%>
					<input class="updateinsert" type="submit" value="change"
						disabled="disabled" /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					
					<button class="inserdelete" type="delete" disabled="disabled"
						onclick="location.href='delete.do?boardno=<%=board.getBoardNo()%>' ">delete</button>
					<%}else{ %>
					<input class="updateinsert" type="submit" value="change" />
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<button class=updatedelete type="button"
						onclick="location.href='delete.do?boardno=<%=board.getBoardNo()%>' ">delete</button>
					<%} %>
