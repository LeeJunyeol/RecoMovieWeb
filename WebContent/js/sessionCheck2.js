	...
	$.post("SessionCheck.do",
			...
			function(data, status){
				if(data == "sessionNull") {	// 로그인 X 
					...
					$("#navbar-item4").html('<a href="#movie" id="movie-link" class="movie-link">Movie</a>');
					$("#navbar-item5").html('<a href="#about" id="member-link" class="member-link">About</a>');
					$("#navbar-item6").html('<a href="reservation.do" id="cinema-link" class="cinema-link">REServation</a>');
					$("#navbar-item7").html('<a href="#team" id="booking-link" class="booking-link">team</a>');
